...
uint8_t Uart_RxTempBuf[MAX_UART_BUFF_SIZE];
uint8_t Uart_TxTempLen;
uint8_t Uart_TxTempBuf[MAX_UART_BUFF_SIZE];
// Uart -> App  Callback
...
void TaskUART_taskInit(void)
{
  ...
  UART_read(UARTHandle, Uart_RxTempBuf, MAX_UART_BUFF_SIZE);       //打开一个串口读
}
...
{ 
  ...
  while(1)
  {
    ...
    {
      UART_read(UARTHandle, Uart_RxTempBuf, MAX_UART_BUFF_SIZE);     //再次打开一个串口读
    }
...
{ 
  ...
  //UART_read(handle, Uart_RxTempBuf, MAX_UART_BUFF_SIZE);    //再次打开一个串口读
